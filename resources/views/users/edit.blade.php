	      ...
	      <div class="my-12">
	        ...
	        <p class="text-sm text-gray-500 mt-4 text-center">Formats : JPG, PNG, JPEG – Max 2 Mo</p>
				...
				<div
						...
						class="space-y-6 flex flex-col items-start justify-center">
					<div>
						<x-label value="Name :" class="pb-3"/>
						<x-input name="name" type="text" class="w-[34rem] !bg-[#f9fafb] pb-3" value="{{ $user->name }}" />
					</div>
					<div>
						<x-label value="Email :" class="pb-3"/>
						<x-input name="email" type="email" class="w-[34rem] !bg-[#f9fafb] pb-3" value="{{ $user->email }}" />
					</div>
					<div>
						<button 
										x-transition
										x-on:click ="open = !open"
										class="rounded text-white p-2 !bg-red-300"
										type="button"
						>
					...
					<div x-show="open" x-transition>
						<x-label value="Nouveau Mot de Passe :" class="pb-3"/>
						<x-input name="password" type="password" class="w-[34rem] !bg-[#f9fafb] pb-3" />
					</div>
					<div x-show="open" x-transition>
						<x-label value="Confirmer Nouveau Mot de Passe :" class="pb-3"/>
						<x-input name="password_confirmation" type="password" class="w-[34rem] !bg-[#f9fafb] pb-3"/>
					</div>
				</div>

				<div class="flex gap-4 my-6">
					<label for="admin" class="relative flex flex-col bg-white px-12 pt-4 rounded-lg cursor-pointer border border-gray-300">
	          <span class="font-semibold text-gray-500 leading-tight">Admin</span>
	          <input type="radio" name="role" id="admin" value="Admin" class="absolute h-0 w-0 appearance-none hidden" />

	          <x-is-checked />
				  </label>

			    <label for="user" class="relative flex flex-col bg-white px-12 py-4 rounded-lg cursor-pointer border border-gray-300">
	          <span class="font-semibold text-gray-500 leading-tight">User</span>
	          ...
	          <x-is-checked />
				...
				<x-button class="my-4 rounded text-white !text-sm py-2 !bg-purple-300 w-[34rem] flex justify-center">Modifier</x-button>
			</div>
		</form>
	</x-container>
<script>
